...
Our first step is to load any required packages and the data. 
To analyze it we will probably need to clean-up some of the data in excel and through R, so we load the tidyverse. 
...
We import the dataset using the qualtric package to enter the data as it organizes it nicely for further analysis. 

I transform all character variables to factors as they are easier to analyse. I also drop some variables from the Qualtrics page that do not have valuable information or are text (need to be codified). 

```{r, message=FALSE}
survey<- read_survey(file_name = "Experimental+Medicine+Student+Society+Survey_August+4,+2021_17.59.csv", 
                     legacy = F,
                     import_id = F
                     ) %>% 
  as_tibble() %>% 
...
colnames(survey)[40:50]

# eliminate unnecessary columns with text
survey1 <- survey %>%  select(Progress, starts_with("Q"), -contains("Text"), -Q_BallotBoxStuffing)
```

## Survey dictionary 

Finally, using the qualtRics package we extract a list of variable descriptions for our reference throughout the analysis
...
Now we move to explore the completion rate of the survey and moving on to characteristics of the respondent:

- type of graduate program (Q17)
- domestic vs international (Q18)
- year of program (Q72)

### Completion rate of the survey

```{r, echo=FALSE}
ggplot(survey, aes(x=as_factor(Progress), fill=as_factor(Progress))) +
  geom_bar() +
  labs(title= "Completion of the survey (n=82)",
       x="Percentage of completion")  +
  theme(legend.position= "none") +
  scale_fill_brewer(palette="Set1")

table (completion_rate=survey$Progress) %>% 
  prop.table() %>% 
  {.*100} %>% 
  round(2)
```

A majority of the 82 participants (61, 74.4%) completed all the survey questions.

### Q17 & Q18: Type of graduate program and student type 
...
In particular, most of the MSc responders 

```{r, include=FALSE}

table(Program=survey1$Q17) %>% 
  prop.table() %>% 
  {.*100} %>% 
  round(1) %>% 
  kable()
...
### Q72: Number of years in the program
...
survey1 %>% 
  filter(!is.na(Q17)) %>% 
  ggplot(aes(x=Q72, fill=as.factor(Q17))) +
  geom_bar() + 
  labs(title="Year of studies according to type of program (n=74)",
...
survey1 %>% 
  filter(!is.na(Q18)) %>% 
  ggplot(aes(x=Q72, fill=as.factor(Q18))) +
  geom_bar() + 
  labs(title="Year of studies according to Domestic Vs. International (n=74)",
       fill="Int vs Domestic",
       x="year of program")+
  facet_wrap(~Q18) +
  scale_fill_brewer(palette = "Set1") +
  theme(axis.text.x = element_text(angle = 90))
...
survey1 %>% 
  ...
  labs(title="Date of program start (n=68)",
       y="Number of students at given start date",
       x="Year of first term",
       colour="Program") +
...
The following questions contain information about student satisfaction in the program and will be summarized by graduate program and international status. 
...
### Q4: How reasonable or unreasonable is the tuition cost at this university?
...
survey1 %>% 
  filter(!is.na(Q17)) %>% 
  ggplot(aes(x=Q4, fill=as.factor(Q17))) +
  geom_bar() + 
  labs(title="Thoughts about the tuition according to program (n=74)")+
  facet_wrap(~Q17) +
  scale_fill_brewer(palette = "Dark2") +
...
survey1 %>% 
  filter(!is.na(Q18)) %>% 
  ggplot(aes(x=Q4, fill=as.factor(Q18))) +
  geom_bar() + 
  labs(title="Thoughts about the tuition Domestic vs International (n=74)")+
  facet_wrap(~Q18) +
  scale_fill_brewer(palette = "Set1") +
  theme(axis.text.x = element_text(angle = 90, size=10),
        legend.position = "none",
...
### Q5: Overall, how satisfied or dissatisfied are you with the ExMed program?
...
table(Satisfaction=survey1$Q5, useNA = "always") %>% 
  addmargins() %>% 
  kable()

survey1 %>% 
  filter(!is.na(Q17)) %>% 
  ggplot(aes(x=Q5, fill=as.factor(Q17))) +
  geom_bar() + 
  labs(title="Satisfaction with the program (n=71)")+
  facet_wrap(~Q17) +
  scale_fill_brewer(palette = "Dark2") +
  theme(axis.text.x = element_text(angle = 90, size=10),
        legend.position = "none",
...
survey1 %>% 
  filter(!is.na(Q18)) %>% 
  ggplot(aes(x=Q5, fill=as.factor(Q18))) +
  geom_bar() + 
  labs(title="Satisfaction with the program (n=71)")+
  facet_wrap(~Q18) +
  scale_fill_brewer(palette = "Set1") +
...
### Q6: How likely are you to recommend this program to students considering graduate school options?
...
survey1 %>% 
  filter(!is.na(Q17)) %>% 
  ggplot(aes(x=Q6_NPS_GROUP, fill=as.factor(Q17))) +
  geom_bar() + 
  labs(title="How likely to recommend the program (n=74)")+
  facet_wrap(~Q17) +
  scale_fill_brewer(palette = "Dark2") +
  theme(axis.text.x = element_text(angle = 90, size=10),
        legend.position = "none",
...
wilcox.test(survey1$Q6~survey1$Q17)


survey1 %>% 
  filter(!is.na(Q17)) %>% 
  ggplot(aes(y=Q6, x=Q17, fill=as.factor(Q17))) +
  geom_boxplot() + 
  geom_jitter(color="black", size=1.7, alpha=0.9) +
  labs(title="How likely to recommend the program (n=74)",
       y="1-Less likely to 10- More likely")+
  scale_fill_brewer(palette = "Dark2") +
  theme(axis.text.x = element_text(angle = 90, size=10),
        legend.position = "none",
        axis.title.x = element_blank())
...
survey1 %>% 
  filter(!is.na(Q18)) %>% 
  ggplot(aes(y=Q6, x=Q18, fill=as.factor(Q18))) +
  geom_boxplot() + 
  geom_jitter(color="black", size=1.7, alpha=0.9) +
  labs(title="How likely to recommend the program (n=74)",
       y="1-Less likely to 10- More likely")+
...
### Q36: Did you attend an ExMed orientation?
...
survey1 %>%
  count(Q36, sort = T) %>%
  mutate(Q36=fct_explicit_na(Q36)) %>% 
  ggplot(aes(fill=Q36, values=n)) +
  geom_waffle(n_rows = 9,
              size = 1.5, 
              colour = "white") +
  labs(title="Did you attend and ExpMed orientation?") +
  scale_fill_manual(name = NULL,
                    values = c("#FF8288", "#BA182A",  "#FFDBDD"),
                    labels = c("No (28%)", "Yes (51.2%)",  "Missing (20.7%)")) +
  coord_equal() +
  theme_void()
```

### Q71: How easy has it been to find information you need about processes and requirements of the program?
...
survey1 %>% 
  filter(!is.na(Q71)) %>% 
  ggplot(aes(x=Q71, fill=factor(Q71))) +
  geom_bar() + 
  labs(title="How easy to find information processes/program (n=65)")+
  scale_fill_brewer(palette = "Dark2") +
  theme(axis.text.x = element_text(angle = 90, size=10),
        legend.position = "none",
        axis.title.x = element_blank())
...
### Q28: What courses have you taken to meet your requirements? 
...
![qualtrics_image_bars](qualtrics_images/courses_taken_bars.png)

### Q34 : Were you able to register in all the courses you were interested in?
...
survey1 %>%
  count(Q34) %>%
  mutate(Q34=fct_explicit_na(Q34)) %>%
  ggplot(aes(fill=Q34, values=n)) +
  geom_waffle(n_rows = 9,
              size = 1.5, 
              colour = "white") +
  labs(title="Able to register in desired courses?") +
  scale_fill_manual(name = NULL,
                    values = c("#BA182A", "#FF8288", "#FFDBDD"),
                    labels = c("Yes (64.6%)", "No (13.4%)", "Missing (22%)")) +
  coord_equal() +
  theme_void()
```
...
### Q35: What was your experience with registering in courses outside of MEDI? 

```{r}
table(how_easy=survey1$Q35, useNA = "always") %>% 
  addmargins() %>% 
  kable()
...
### Q29: What was your overall satisfaction with the courses you took?

```{r}
survey1 %>% 
  filter(!is.na(Q29)) %>% 
  ggplot(aes(x=Q29, fill=factor(Q29))) +
  geom_bar() + 
  labs(title="Satisfaction with courses taken? (n=61)")+
  scale_fill_brewer(palette = "Dark2") +
  theme(axis.text.x = element_text(angle = 90, size=10),
        legend.position = "none",
        axis.title.x = element_blank())
...
### Q33: Do you feel that there should be more MEDI electives, i.e. courses that are not MEDI 501/502/503?

```{r}
table(more_MEDI_electives=survey1$Q33, useNA = "always") %>% 
  addmargins() %>% 
  kable()

survey1 %>%
  count(Q33) %>%
  mutate(Q33=fct_explicit_na(Q33)) %>%
  ggplot(aes(fill=Q33, values=n)) +
  geom_waffle(n_rows = 9,
              size = 1.5, 
              colour = "white") +
  labs(title="Do you feel that there should be more MEDI electives?") +
  scale_fill_manual(name = NULL,
                    values = c("#BA182A", "#FF8288", "#FFDBDD"),
                    labels = c("Yes (57.3%)", "No (18.3%)", "Missing (24.4%)")) +
  coord_equal() +
  theme_void()
```
...
### Q57 - We are interested in hearing about your relationship with your supervisor

```{r}
q57<- read_csv(file = "Q57_supervisor_relationship.csv") %>%
  pivot_longer(
...
slice(q57, 1:15)  %>% 
  ...
  labs(title=("Less (1) to complete (5) agreement"),
       fill=NULL)
...
### Q20: What average range of funding (CAD$) have you been receiving considering external funds combined?
...
survey1 %>% 
  filter(!is.na(Q20)) %>% 
  ggplot(aes(x=Q20, fill=factor(Q20))) +
  geom_bar() + 
  labs(title="Range of funding received (n=61)",
       y="Thousands of CAD/year")+
  scale_fill_brewer(palette = "Dark2") +
  facet_wrap(~Q17) +
...
### Q27: How often do you worry about funding/financial support for your studies and living?
...
survey1 %>% 
  filter(!is.na(Q27)) %>% 
  ggplot(aes(x=Q27, fill=factor(Q27))) +
  geom_bar() + 
  labs(title="How often do you worry about funding (n=61)",
       y="Counts")+
  scale_fill_brewer(palette = "Dark2") +
  facet_wrap(~Q17) +
  theme(axis.text.x = element_text(angle = 90, size=9),
...
![qualtrics_image_bars](qualtrics_images/funding_applied.png)
...
![qualtrics_image_bars](qualtrics_images/funding_received.png)
...
### Q77 - The COVID19 pandemic has affected my overall mood and mental well being?
...
survey1 %>% 
ggplot(aes(x=Q77, fill=Q77) ) +
  geom_bar() +
  scale_fill_brewer(palette = "OrRd") +
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank()) +
  theme(legend.position="bottom") +
  labs(title="The COVID19 pandemic has affected my overall mood and mental well being?",
       x="", 
       fill="")
...
### Q67 - Series of questions about impact of covid in education
...
```{r}
q67 <-  read_csv(file = "q67_pandemic_experience.csv") %>% 
  pivot_longer(
    cols = -c(Question, Total),
    names_to = "score",
    values_to = "count"
...
### Q55 - The following question asks about well-being, mental-health and academic resources available at UBC

__Resources known by the students__

In the following 2 graphs, we have divided the most common resources available for UBC students and show the number of students that have heard about them. 

![resources_known1](qualtrics_images/known_resources_1.png)

![resources_known2](qualtrics_images/known_resources_2.png)
From these graphs we can see that the most well-known resources to students in our program are UBC counselling, UBC health services, Library research commons and the workshops provided by G+PS. Resources for sexual violence or sexual assault are widely unknown according to the survey. Also, the less known service is the mistreatment office of the faculty of medicine. 

__Evaluation of services__
...
```{r}
q55 <-  read_csv(file = "q55_evaluation_resources_used.csv") %>% 
  pivot_longer(
    cols = -c(Question, Total),
    names_to = "score",
    values_to = "count"
...
ggplot( data=q55, aes(x=score, y=count, fill=score) ) +
  geom_histogram(stat = "identity", position = "dodge") +
  facet_grid(~Question) +
  scale_fill_brewer(palette = "BuGn") +
  theme(axis.text.x = element_blank(),
...
```{r}
q61<- read_csv(file = "q61_feeling_program.csv") %>%
  pivot_longer(
    cols = -c(Question, Total),
    names_to = "score",
    values_to = "count"
