...
--- 
```
knitr::opts_chunk$set(echo = F, warning = F, message = F, eval=T, fig.align = 'center')
```
...
```{r, include=FALSE}
# produces a frequency table (kbl formatted) with n and frequencies

kbl_count_wrapper <- function (data, var, total=82) {
  count(data, {{var}}) %>% 
    mutate(freq=n/total*100) %>%
  kbl(., format="html", align = c("lcccc"), row.names = NA, digits=2) %>%
...
```{r, geom_bar wrapper, include=FALSE, results='hide'}

geom_bar_wrapper <- function (data, x=Q72, group=Q17, 
                              plot.title="title", x.title="", 
                              facet_label="top", palette="red") 
  {
plot<-  ggplot(data, aes(x={{x}},fill=as.factor({{group}}))) + geom_bar()
plot <-  plot +  facet_wrap(vars({{group}}), scales = "free_x", strip.position=facet_label)
...
survey %>% 
  mutate(Progress=case_when(Progress<100 ~ "Partially complete", 
                            TRUE~ "Fully complete")) %>%
  kbl_count_wrapper(. , var=Progress)
```
...
survey1 %>% 
  filter(!is.na(Q17)) %>%  
  geom_bar_wrapper(palette="green")
```


```{r}
filter(survey1, !is.na(Q17)) %>% 
  geom_bar_wrapper(plot.title ="Year in ExpMed graduate program (n=74)")
```
